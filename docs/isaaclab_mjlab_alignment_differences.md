...
| Leg symmetry | 左右镜像姿态约束 | 不是 trot 相位约束 | 保留但需谨慎解释 | 不能保证 `FL+HR / FR+HL` 对角步态 |
...
### 6. Leg symmetry 不是 trot symmetry

`leg_symmetry` 主要约束左右镜像，例如：
...
因此它不是严格的 trot 相位奖励。此前观察到“前两条腿同步、后两条腿拖行”时，这种动作仍可能部分满足左右对称。后续没有继续加显式 trot 约束，是因为 footobs 版本已经自然出现了较好的步态，继续大改会破坏当前有效方向。
